                    ...
                    success: function (data) {
                        if (data.length > 0) {
                            ...
                            data.forEach(function (e) {
                                resultList += '<tr data-appeal-number="' + e.number_appeal + '">';
                                resultList += '<td class="text-left">';
                                resultList += '<span class="font-12 text-nowrap">' + e.applicant_name + '</span>';
                                resultList += '<b class="d-block my-1 lh-12">' + e.text_appeal + '</b>';
                                resultList += '<p class="mb-0 font-12">';
                                resultList += '<span>#' + e.number_appeal + '</span> от <span class="m-r-5">' + moment(e.date_add).format('DD.MM.YYYY') + '</span>';
                                resultList += '<span class="label label-default">' + e.status_name + '</span>';
                                resultList += '</td>';
                                resultList += '</tr>';
                            });
  ...
  var onCallBtnClick = function () {
      ...
      if (callNumber.value) {
          ...
          $.ajax({
              ...
              data: { phone_number: callNumber.value, number_appeal:d },
          ...
          $.ajax({
              async: true,
              url: '/Call/UserInfo',
              data: { phone_number: callNumber.value},
              type: 'POST',
              success: function (data) {
...
//const callModalBtnFromAppeal = document.querySelector('#btnCallModalFromAppeal');
//callModalBtnFromAppeal.addEventListener('click', onCallModalBtnClick);
...
function setupWebSocket() {
    this.ws = new WebSocket('ws://localhost:8886');
    this.ws.onerror = function (event) { console.log("error socket", event); };
    //ws.onerror = ev => { ev.stopImmediatePropagation(); ev.stopPropagation(); ev.preventDefault(); return false }
    this.ws.onopen = function (event) { console.log("open socket"); };
